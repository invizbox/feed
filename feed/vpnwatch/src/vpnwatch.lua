...
function vpnwatch.setup_vpn()
    ...
    local filename = "/tmp/openvpn.conf"
    if uci:get(config_name, active_vpn) == "server" then
        if uci:get(config_name, active_vpn, "template") then
            local template = uci:get(config_name, active_vpn, "template")
            local address = uci:get(config_name, active_vpn, "address")
            os.execute('sed "s/@SERVER_ADDRESS@/'..address..'/" '..template..' > '..filename)
        elseif uci:get(config_name, active_vpn, "filename") then
            filename = uci:get(config_name, active_vpn, "filename")
        end

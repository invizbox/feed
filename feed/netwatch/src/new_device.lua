...
local function add_device(section_name, mac_address, ip_address, device_name)
    local config_name = "known_devices"
    ...
    local previous_device_name = uci:get(config_name, section_name, "device_name")
    if previous_device_name == nil or previous_device_name == "" then
        uci:set(config_name, section_name, "device_name", device_name)
    end

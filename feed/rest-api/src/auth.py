...
def access_token(uci):
    ...
    try:
        ...
        return {"token": new_access_tk.decode("utf-8"), "expires": str(expires), "refresh_token_id": token["id"]}
...
@AUTH_APP.post('/auth/refresh_token')
@jwt_auth_required
def new_refresh_token(uci):
    ...
    try:
        ...
        response.set_cookie('api_refresh', created_token["token"], path='/api/auth/access_token', domain=host,
...
@AUTH_APP.put('/auth/revoke_refresh_token/<token_id>')
@jwt_auth_required

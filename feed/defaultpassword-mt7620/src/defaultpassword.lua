...
function default_password.get_vpn_credentials()
    local username = utils.run_and_log("dd if=/dev/mtd2 bs=1 skip=65434 count=64 2>/dev/null")
    local password = utils.run_and_log("dd if=/dev/mtd2 bs=1 skip=65498 count=24 2>/dev/null")
    return username:gsub("%s*$", ""), password:gsub("%s*$", "")
end

function default_password.get_wifi_password()
    return utils.run_and_log("dd if=/dev/mtd2 bs=1 skip=65522 count=14 2>/dev/null")
end

function default_password.set_vpn_credentials(username, password)
    local result = os.execute(". /bin/mtdutils.sh; write_string_to_mtd2 '"..username.."' 64 65434")
    local result2 = os.execute(". /bin/mtdutils.sh; write_string_to_mtd2 '"..password.."' 24 65498")
    return result, result2
end

function default_password.set_wifi_password(password)
    return os.execute(". /bin/mtdutils.sh; write_string_to_mtd2 '"..password.."' 14 65522")
end

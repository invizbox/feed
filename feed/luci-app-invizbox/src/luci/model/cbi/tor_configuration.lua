...
function tor_stop_button.write(_, _)
    sys.call('logger "stopping tor"; /etc/init.d/tor stop')
end

local tor_new_identity = section:taboption("torstatus", cbi.Button, "_identity", "")
tor_new_identity.inputtitle = translate.translate("New Identity")
tor_new_identity.template = "cbi/end_button"
...
function tor_status.cfgvalue()
    ...
    if sock and sock:connect("127.0.0.1", 9051) then
        ...
        res, data = utils.tor_request(sock, "GETINFO status/circuit-established\r\n")
        if not res then
            return false
        end
        ...
        if status == "=1" then
            ...
            res, data = utils.tor_request(sock, "GETINFO status/bootstrap-phase\r\n")
            if not res then
                return false
            else
...
tor_circ.template = "cbi/label"
tor_circ.scroll = true
...
--[[======================= BRIDGE CONFIG ===============]]--
local enter_string = "Please enter in the bridges you want Tor to use, one per line.<br>The format is : \"ip:port [fi"..
        "ngerprint]\" where fingerprint is optional. e.g. 121.101.27.4:443 4352e58420e68f5e40ade74faddccd9d1349413.<b"..
        "r> To get bridge information, see <a href=\"https://bridges.torproject.org/bridges\">the Tor bridges page</a"..
        ">.<br><br>InvizBox also has pluggable transport support. We support  <a href=\"https://bridges.torproject.or"..
        "g/bridges?transport=obfs2\">obfs2</a>,  <a href=\"https://bridges.torproject.org/bridges?transport=obfs3\">o"..
        "bfs3</a> and <a href=\"https://bridges.torproject.org/bridges?transport=scramblesuit\">scramblesuit</a> brid"..
        "ges. Only use these bridge types if normal bridges are blocked for you."
local bridge_config = section:taboption("bridge", cbi.TextValue, "_data", translate.translate("Bridge Configuration"),
    translate.translate(enter_string))
...
function map.on_commit()
    if bNewIdentity == 0 then
        sys.call("cat /rom/etc/tor/torrc /etc/tor/proxy /etc/tor/bridges /etc/tor/geoip > /etc/tor/torrc ; "..
